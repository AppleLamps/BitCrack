...
/* Four independent blocks, interleaved so SHA-NI latency overlaps. */
SHA256_SHANI_TARGET
static inline void sha256_process_words4(uint32_t s0[8], const uint32_t w0[16],
	uint32_t s1[8], const uint32_t w1[16],
	uint32_t s2[8], const uint32_t w2[16],
	uint32_t s3[8], const uint32_t w3[16])
{
	__m128i STATE0[4], STATE1[4];
	__m128i MSG[4], TMP[4];
	__m128i MSG0[4], MSG1[4], MSG2[4], MSG3[4];
	__m128i ABEF_SAVE[4], CDGH_SAVE[4];
	const uint32_t *w[4] = { w0, w1, w2, w3 };
	uint32_t *state[4] = { s0, s1, s2, s3 };

#define SHA4(...) do { \
	{ const int sha_i = 0; __VA_ARGS__; } \
	{ const int sha_i = 1; __VA_ARGS__; } \
	{ const int sha_i = 2; __VA_ARGS__; } \
	{ const int sha_i = 3; __VA_ARGS__; } \
} while(0);



	SHA4(TMP[sha_i] = _mm_loadu_si128((const __m128i *)&state[sha_i][0]);)
	SHA4(STATE1[sha_i] = _mm_loadu_si128((const __m128i *)&state[sha_i][4]);)

	SHA4(TMP[sha_i] = _mm_shuffle_epi32(TMP[sha_i], 0xB1);)
	SHA4(STATE1[sha_i] = _mm_shuffle_epi32(STATE1[sha_i], 0x1B);)
	SHA4(STATE0[sha_i] = _mm_alignr_epi8(TMP[sha_i], STATE1[sha_i], 8);)
	SHA4(STATE1[sha_i] = _mm_blend_epi16(STATE1[sha_i], TMP[sha_i], 0xF0);)

	SHA4(ABEF_SAVE[sha_i] = STATE0[sha_i];)
	SHA4(CDGH_SAVE[sha_i] = STATE1[sha_i];)

	SHA4(MSG0[sha_i] = _mm_loadu_si128((const __m128i *)(w[sha_i] + 0));)
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG0[sha_i], _mm_set_epi64x(0xE9B5DBA5B5C0FBCFULL, 0x71374491428A2F98ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)

	SHA4(MSG1[sha_i] = _mm_loadu_si128((const __m128i *)(w[sha_i] + 4));)
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG1[sha_i], _mm_set_epi64x(0xAB1C5ED5923F82A4ULL, 0x59F111F13956C25BULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)
	SHA4(MSG0[sha_i] = _mm_sha256msg1_epu32(MSG0[sha_i], MSG1[sha_i]);)

	SHA4(MSG2[sha_i] = _mm_loadu_si128((const __m128i *)(w[sha_i] + 8));)
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG2[sha_i], _mm_set_epi64x(0x550C7DC3243185BEULL, 0x12835B01D807AA98ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)
	SHA4(MSG1[sha_i] = _mm_sha256msg1_epu32(MSG1[sha_i], MSG2[sha_i]);)

	SHA4(MSG3[sha_i] = _mm_loadu_si128((const __m128i *)(w[sha_i] + 12));)
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG3[sha_i], _mm_set_epi64x(0xC19BF1749BDC06A7ULL, 0x80DEB1FE72BE5D74ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG3[sha_i], MSG2[sha_i], 4);)
	SHA4(MSG0[sha_i] = _mm_add_epi32(MSG0[sha_i], TMP[sha_i]);)
	SHA4(MSG0[sha_i] = _mm_sha256msg2_epu32(MSG0[sha_i], MSG3[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)
	SHA4(MSG2[sha_i] = _mm_sha256msg1_epu32(MSG2[sha_i], MSG3[sha_i]);)

	SHA4(MSG[sha_i] = _mm_add_epi32(MSG0[sha_i], _mm_set_epi64x(0x240CA1CC0FC19DC6ULL, 0xEFBE4786E49B69C1ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG0[sha_i], MSG3[sha_i], 4);)
	SHA4(MSG1[sha_i] = _mm_add_epi32(MSG1[sha_i], TMP[sha_i]);)
	SHA4(MSG1[sha_i] = _mm_sha256msg2_epu32(MSG1[sha_i], MSG0[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)
	SHA4(MSG3[sha_i] = _mm_sha256msg1_epu32(MSG3[sha_i], MSG0[sha_i]);)

	SHA4(MSG[sha_i] = _mm_add_epi32(MSG1[sha_i], _mm_set_epi64x(0x76F988DA5CB0A9DCULL, 0x4A7484AA2DE92C6FULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG1[sha_i], MSG0[sha_i], 4);)
	SHA4(MSG2[sha_i] = _mm_add_epi32(MSG2[sha_i], TMP[sha_i]);)
	SHA4(MSG2[sha_i] = _mm_sha256msg2_epu32(MSG2[sha_i], MSG1[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)
	SHA4(MSG0[sha_i] = _mm_sha256msg1_epu32(MSG0[sha_i], MSG1[sha_i]);)

	SHA4(MSG[sha_i] = _mm_add_epi32(MSG2[sha_i], _mm_set_epi64x(0xBF597FC7B00327C8ULL, 0xA831C66D983E5152ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG2[sha_i], MSG1[sha_i], 4);)
	SHA4(MSG3[sha_i] = _mm_add_epi32(MSG3[sha_i], TMP[sha_i]);)
	SHA4(MSG3[sha_i] = _mm_sha256msg2_epu32(MSG3[sha_i], MSG2[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)
	SHA4(MSG1[sha_i] = _mm_sha256msg1_epu32(MSG1[sha_i], MSG2[sha_i]);)

	SHA4(MSG[sha_i] = _mm_add_epi32(MSG3[sha_i], _mm_set_epi64x(0x1429296706CA6351ULL, 0xD5A79147C6E00BF3ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG3[sha_i], MSG2[sha_i], 4);)
	SHA4(MSG0[sha_i] = _mm_add_epi32(MSG0[sha_i], TMP[sha_i]);)
	SHA4(MSG0[sha_i] = _mm_sha256msg2_epu32(MSG0[sha_i], MSG3[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG0[sha_i], _mm_set_epi64x(0x53380D134D2C6DFCULL, 0x2E1B213827B70A85ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG0[sha_i], MSG3[sha_i], 4);)
	SHA4(MSG1[sha_i] = _mm_add_epi32(MSG1[sha_i], TMP[sha_i]);)
	SHA4(MSG1[sha_i] = _mm_sha256msg2_epu32(MSG1[sha_i], MSG0[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG1[sha_i], _mm_set_epi64x(0x92722C8581C2C92EULL, 0x766A0ABB650A7354ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG1[sha_i], MSG0[sha_i], 4);)
	SHA4(MSG2[sha_i] = _mm_add_epi32(MSG2[sha_i], TMP[sha_i]);)
	SHA4(MSG2[sha_i] = _mm_sha256msg2_epu32(MSG2[sha_i], MSG1[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG2[sha_i], _mm_set_epi64x(0xC76C51A3C24B8B70ULL, 0xA81A664BA2BFE8A1ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG2[sha_i], MSG1[sha_i], 4);)
	SHA4(MSG3[sha_i] = _mm_add_epi32(MSG3[sha_i], TMP[sha_i]);)
	SHA4(MSG3[sha_i] = _mm_sha256msg2_epu32(MSG3[sha_i], MSG2[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG3[sha_i], _mm_set_epi64x(0x106AA070F40E3585ULL, 0xD6990624D192E819ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG3[sha_i], MSG2[sha_i], 4);)
	SHA4(MSG0[sha_i] = _mm_add_epi32(MSG0[sha_i], TMP[sha_i]);)
	SHA4(MSG0[sha_i] = _mm_sha256msg2_epu32(MSG0[sha_i], MSG3[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG0[sha_i], _mm_set_epi64x(0x34B0BCB52748774CULL, 0x1E376C0819A4C116ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG0[sha_i], MSG3[sha_i], 4);)
	SHA4(MSG1[sha_i] = _mm_add_epi32(MSG1[sha_i], TMP[sha_i]);)
	SHA4(MSG1[sha_i] = _mm_sha256msg2_epu32(MSG1[sha_i], MSG0[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG1[sha_i], _mm_set_epi64x(0x682E6FF35B9CCA4FULL, 0x4ED8AA4A391C0CB3ULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG1[sha_i], MSG0[sha_i], 4);)
	SHA4(MSG2[sha_i] = _mm_add_epi32(MSG2[sha_i], TMP[sha_i]);)
	SHA4(MSG2[sha_i] = _mm_sha256msg2_epu32(MSG2[sha_i], MSG1[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG2[sha_i], _mm_set_epi64x(0x8CC7020884C87814ULL, 0x78A5636F748F82EEULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(TMP[sha_i] = _mm_alignr_epi8(MSG2[sha_i], MSG1[sha_i], 4);)
	SHA4(MSG3[sha_i] = _mm_add_epi32(MSG3[sha_i], TMP[sha_i]);)
	SHA4(MSG3[sha_i] = _mm_sha256msg2_epu32(MSG3[sha_i], MSG2[sha_i]);)
	...
	SHA4(MSG[sha_i] = _mm_add_epi32(MSG3[sha_i], _mm_set_epi64x(0xC67178F2BEF9A3F7ULL, 0xA4506CEB90BEFFFAULL));)
	SHA4(STATE1[sha_i] = _mm_sha256rnds2_epu32(STATE1[sha_i], STATE0[sha_i], MSG[sha_i]);)
	SHA4(MSG[sha_i] = _mm_shuffle_epi32(MSG[sha_i], 0x0E);)
	SHA4(STATE0[sha_i] = _mm_sha256rnds2_epu32(STATE0[sha_i], STATE1[sha_i], MSG[sha_i]);)

	SHA4(STATE0[sha_i] = _mm_add_epi32(STATE0[sha_i], ABEF_SAVE[sha_i]);)
	SHA4(STATE1[sha_i] = _mm_add_epi32(STATE1[sha_i], CDGH_SAVE[sha_i]);)

	SHA4(TMP[sha_i] = _mm_shuffle_epi32(STATE0[sha_i], 0x1B);)
	SHA4(STATE1[sha_i] = _mm_shuffle_epi32(STATE1[sha_i], 0xB1);)
	SHA4(STATE0[sha_i] = _mm_blend_epi16(TMP[sha_i], STATE1[sha_i], 0xF0);)
	SHA4(STATE1[sha_i] = _mm_alignr_epi8(STATE1[sha_i], TMP[sha_i], 8);)

	SHA4(_mm_storeu_si128((__m128i *)&state[sha_i][0], STATE0[sha_i]);)
	SHA4(_mm_storeu_si128((__m128i *)&state[sha_i][4], STATE1[sha_i]);)

#undef SHA4
}

SHA256_SHANI_TARGET
static inline void sha256_process_iv4(uint32_t s0[8], const uint32_t w0[16],
	uint32_t s1[8], const uint32_t w1[16],
	uint32_t s2[8], const uint32_t w2[16],
	uint32_t s3[8], const uint32_t w3[16])
{
	...
	for(int i = 0; i < 8; i++) {
		a[i] = b[i] = c[i] = d[i] = IV[i];
	}
	sha256_process_words4(a, w0, b, w1, c, w2, d, w3);
	for(int i = 0; i < 8; i++) {
		s0[i] = a[i];
		s1[i] = b[i];
		s2[i] = c[i];
...
SHA256_SHANI_TARGET
void crypto_sha256_shani4(unsigned int *msg0, unsigned int *digest0,
	unsigned int *msg1, unsigned int *digest1,
	unsigned int *msg2, unsigned int *digest2,
	unsigned int *msg3, unsigned int *digest3)
{
	sha256_process_words4(digest0, msg0, digest1, msg1, digest2, msg2, digest3, msg3);
}

SHA256_SHANI_TARGET
void crypto_sha256_shani4_iv(unsigned int *msg0, unsigned int *digest0,
	unsigned int *msg1, unsigned int *digest1,
	unsigned int *msg2, unsigned int *digest2,
	unsigned int *msg3, unsigned int *digest3)
{
	sha256_process_iv4(digest0, msg0, digest1, msg1, digest2, msg2, digest3, msg3);
}
}
#endif
