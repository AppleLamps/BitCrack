...
`-t` is worker threads. `-p` is herd size (default 64 per thread). `--dp` sets distinguished-point bits (0 = auto). `--kangaroo-opt N` selects optional solvers (`1` batched mixed-adds, `2` retuned jumps, `4` herd-aware distinguished points; values can be added together). `--kangaroo-bench` A/B tests those variants against the baseline.
